...
def train_dino(args,cf):
    ...
    dataset = HiDiscDINO(
        ...
        studies="train",
        transform=Compose(get_srh_aug_list_dino(cf["data"]["train_augmentation"])),
...
class DINOLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        student_out = student_out.chunk(self.ncrops*4)

        # teacher centering and sharpening
        temp = self.teacher_temp_schedule[epoch]
        teacher_out = F.softmax((teacher_output - self.center) / temp, dim=-1)
        teacher_out = teacher_out.detach().chunk(8)

        total_loss = 0
        n_loss_terms = 0
        for iq, qx in enumerate(teacher_out):
            for v in range(len(student_out)):
                if v == iq:
                    # we skip cases where student and teacher operate on the same view
                    continue
                loss = torch.sum(-qx * F.log_softmax(student_out[v], dim=-1), dim=-1)
