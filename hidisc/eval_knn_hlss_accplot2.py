...
from common import (parse_args, get_exp_name, config_loggers,
                           get_num_worker)
from train_hlss_3mlp import HiDiscSystem
...
def make_specs(predictions: Dict[str, Union[torch.Tensor, List[str]]]) -> None:
    ...
    csv_filename = "exp9b_eval.csv"
    
    with open(csv_filename, mode='a', newline='') as file:
        writer = csv.writer(file)

        # Write a new row with the provided values
        writer.writerow([get_all_metrics(patch_logits, patch_label)[0], get_all_metrics(slides_logits, slides_label)[0], get_all_metrics(patient_logits,patient_label)[0]])
