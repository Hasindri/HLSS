...
from models import MLP, resnet_backbone, ContrastiveLearningNetwork, HLSSContrastiveLearningNetwork, HLSSHidiscNetwork, CLIPTextClassifier,CLIPVisual, ResnetHLSSNetwork
from common import (setup_output_dirs, parse_args, get_exp_name,
                           config_loggers, get_optimizer_func,
                           get_scheduler_func, get_dataloaders,get_dataloaders_hidisc)
...
class HiDiscSystem(pl.LightningModule):
    ...
    def __init__(self, cf: Dict[str, Any], num_it_per_ep: int,freeze_mlp: bool):
        ...
        if cf["model"]["backbone"] == "RN50":
            ...
            bb = partial(resnet_backbone, arch="resnet50")

        else:
            raise NotImplementedError()

        mlp2 = partial(CLIPTextClassifier,
                      n_in=1024,
        ...
        self.model = ResnetHLSSNetwork(bb, mlp1, mlp2)
...
def main():
    ...
    trainer.fit(exp,
                ...
                val_dataloaders=valid_loader, ckpt_path = "/data1/dri/hidisc/hidisc/exps/Exp015/a/c0a863b8-Jan25-00-03-48-patient_disc_dev_/models/ckpt-epoch12799.ckpt")
