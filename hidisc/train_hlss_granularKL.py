...
from models import MLP, HLSSGranularKL, HLSSHidiscNetwork,resnet_backbone, ContrastiveLearningNetwork, HLSSContrastiveLearningNetwork, CLIPTextClassifier,CLIPVisual,HLSSKL
...
class HiDiscSystem(pl.LightningModule):
    ...
    def __init__(self, cf: Dict[str, Any], num_it_per_ep: int,freeze_mlp: bool):
        ...
        if cf["model"]["backbone"] == "RN50":
            bb1 = partial(CLIPVisual, arch=cf["model"]["backbone"])
            bb2 = partial(CLIPVisual, arch=cf["model"]["backbone"])
        else:
        ...
        self.model = HLSSGranularKL(bb1, bb2, mlp1,mlp2,mlp3)
    ...
    def predict_step(self, batch, batch_idx):
        ...
        out = self.model.visualbb(batch["image"])
        return {
            "path": batch["path"],
            "label": batch["label"],
            "embeddings": out
...
def main():
    ...
    trainer.fit(exp,
                ...
                val_dataloaders=valid_loader, ckpt_path = "/data1/dri/hidisc/hidisc/exps/Exp020/b/0bcbf6e6-Feb19-10-45-00-patient_disc_dev_/models/ckpt-epoch1199.ckpt")
