...
class OpenSRHDataset(Dataset):
    ...
    #     except OSError as e:
    #         logging.error(f"Error reading image at index {idx}: {e}")
    #         # If the image reading fails, close any opened file if applicable
    #         try:
    #             if 'im' in locals() and hasattr(im, 'close'):
    #                 im.close()
    #         except Exception as close_exception:
    #             logging.warning(f"Error closing file: {close_exception}")

    #         # Return None or any other suitable value to indicate a skipped entry
    #         return None
